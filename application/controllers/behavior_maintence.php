...
class behavior_maintence extends BM_Controler {
    ...
    protected function form_common() {
        ...
        $this->bm_form_builder->set_extra_for('behavior.name', ['placeholder' => '', 'class' => 'form-control']);
    }
    ...
    protected function get_data($hook, $data): array {
        if ($hook === 'lista') {
            ...
            $this->bm_form_builder->hide_form_values(['behavior.id']);
            $data = array_merge($data, [
                'form_title' => $this->lang->line('Edit Behavior'),
                'form_attributes' => ['class' => 'form-inline'],
            ]);
        } elseif ($hook === 'novo') {
            $this->bm_form_builder->exclude_form_values(['behavior.id']);
            $data = array_merge($data, [
